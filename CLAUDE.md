...
**Nullifier v4** is a multi-agent scientific hypothesis stress-tester with a web UI and CLI. It extracts falsifiable claims, retrieves evidence from four federated databases, cross-checks against Ensembl genomic data (biology hypotheses), and delivers a structured verdict. Hybrid LLM routing: per-paper classification via LM Studio (local), reasoning via Claude.
...
hypothesis.txt
    → pipeline.py (event generator)
        ...
        → Analyst    (Ensembl genomic data, biology only)
...
**v5 — completed-analysis critique**: Formalizer Stage 1 also extracts optional `methods_used` and `completed_analysis` (`[{finding, statistic, test, sample_size, interpretation}]`). When `completed_analysis` is non-empty, the pipeline emits `formalizer_detected_completed_analysis`, the Analyst runs `_reproducibility_check` (assembles Ensembl-retrievable per-gene values + an honest `not_verifiable_here` list; the LLM reconciles into `interpretation.reproducibility_check`), and the Skeptic appends `SKEPTIC_CRITIQUE_BLOCK` → adds `methods_critique` / `statistical_critique` / `reproducibility_check` / `interpretation_critique` (`{severity, issues[], notes}`), four `*_critique_score`/`reproducibility_score` rows, and the verdict `RESULTS-PROBLEMATIC` (overrides `NOVEL-UNTESTED` on high-severity issues). Flags are now also exposed over HTTP (`GET/POST /api/flags`, `GET /api/flags/export`) and have a full web UI (inline flag in `EvidencePanel`, `/runs/:id/review`, the `/flags` library).
...
**`tools/ensembl.py`** — Ensembl REST client. SQLite cache at `~/.nullifier/ensembl_cache.db`. 10 req/s rate limit. Returns `None` on failure (never throws).

**`agents/analyst.py`** — Splits starter entities into Set A (synaptic) and Set B (BBB) via GO terms. Fetches orthologs, dN/dS, regulatory features, motif features. Computes cross-set Jaccard. Calls Claude to interpret. Only runs when `domain in BIOLOGY_DOMAINS` and `starter_entities` is non-empty.

**`frontend/`** — Vite + React + TypeScript + Recharts. Tailwind v4 is imported (`@import "tailwindcss"` in `index.css`) but only a few utilities are used (e.g. `animate-spin`); page/component styling is **inline styles + CSS custom properties** defined in `index.css` (`--oxford`, `--bg`, `--surface`, `--border`, `--text-*`, `--verdict-*`) — an "academic dashboard" theme with dark `#0f172a` headers. `RunPage` is a single scrolling document (no tabs): Hypothesis → Evidence → Genomic → Verdict. `vite.config.ts` proxies `/api` and `/ws` to `:8000` in dev. Production build goes to `backend/nullifier/static/`.
